...
class Teleduino328
{
	...
	protected function _processEthernetClientProxy()
	{
		...
		$port = $this->_modeSettings['ethernetClientProxy']['port'];
		...
		$url = 'http://'.$address.':'.$port.'/api/'.$version.'/basic.php';
	...
	protected function _processEthernetServerWeb()
	{
		...
		$port = $this->_modeSettings['ethernetServerWeb']['port'];
		$data_hex = $this->_dataHexEncode();
		$url = 'http://'.$address.':'.$port.'/'.$data_hex;
	...
	public function defineServo($servo, $pin, $min, $max)
	{
		...
		if (isset($min)) { $this->_data[4] = chr($min); };
		if (isset($max)) { $this->_data[5] = chr($max); };
		$start = microtime(true);
		if(!$this->_process())
		{
			return false;
	...
	{
		$address = isset($config['address']) ? $config['address'] : 'us01.proxy.teleduino.org';
		...
		$this->_modeSettings['ethernetClientProxy']['port'] = $port;
	...
	public function setModeEthernetServerWeb($config)
	{
		...
		$this->_modeSettings['ethernetServerWeb']['port'] = $port;
		return true;
	}
